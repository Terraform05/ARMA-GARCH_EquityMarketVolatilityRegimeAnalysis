...
from src.layered_strategy import build_layered_frame
...
def _plot_outperformance(
    ...
    layered_out = (pivot["layered"] > pivot["benchmark"]).mean()
    regime_out = (pivot["regime"] > pivot["benchmark"]).mean()

    fig, ax = plt.subplots(figsize=(6, 4))
    ax.bar(["layered", "regime"], [layered_out, regime_out], color=["slateblue", "#7E57C2"])
    ax.set_ylim(0, 1)
...
def run_gbm_forward_test(
    *,
    ...
    / "data"
    / "regime_series.csv",
    output_dir: Path = PROJECT_ROOT / "reports" / "strategy_layered_gbm",
    ...
    for horizon in horizons_years:
        ...
        for path_idx, log_returns in enumerate(paths):
            ...
            layered_frame = build_layered_frame(
                sim,
