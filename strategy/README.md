...
- Run walk-forward optimization with Sortino (net of costs) and a drawdown cap to select hyperparameters.
- Freeze the best config to `configs/regime_trend_best.json`.
- The regime-trend backtest uses the frozen config automatically when it exists.

## Modular Components

- `src/signals/`: signal calculators (trend, volatility, etc.)
- `src/strategies/`: strategy assembly and sizing logic
- `build_regime_trend_frame(...)` accepts optional `trend_score`, `trend_state`, or
  `vol_score` so new signals can be injected without rewiring the pipeline.
...
Runner (writes ranked tables to `reports/strategy_regime_trend_sweep/`):
- Run `scripts/run_regime_trend_sweep.py` directly in your IDE.
- Edit defaults in `scripts/run_regime_trend_sweep.py` under `_default_config()`
  if you want to change the sweep grid or dates.

Inspect:
- `reports/strategy_regime_trend_sweep/data/sweep_results.csv` for all configs
- `reports/strategy_regime_trend_sweep/data/top_candidates.csv` for the top ranked configs
- `reports/strategy_regime_trend_sweep/data/sweep_summary.txt` for the sweep setup
