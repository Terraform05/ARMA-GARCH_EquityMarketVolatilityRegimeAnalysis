...
things look risky or falling.** The regime-trend approach aims to capture upside
...
| --- | --- | --- | --- | --- | --- |
| Regime-Trend (gross) | 18.25% | 9.51% | 1.92 | -9.03% | Best risk-adjusted balance |
| Regime-Trend (net, 5 bps costs) | 17.26% | 9.50% | 1.82 | -9.08% | Cost drag is modest |
...
- **Regime-Trend gives up a little return for much smoother risk.**
...
![Equity curve comparison (last year)](../reports/strategy_regime_trend/plots/equity_curve_compare_last_year.png)

What to notice:
- **Trend-only is highest, but regime-trend is close with smoother behavior.**
- **Regime-only stays low (defensive but lower return).**
- **Regime-Trend beats buy-and-hold while taking less risk.**

### 4) Last-year regime-trend vs buy-and-hold (simpler view)
![Equity curve (last year)](../reports/strategy_regime_trend/plots/equity_curve_last_year.png)

What to notice:
- **Regime-Trend should dip less during drawdowns and recover faster.**
- The gap shows how much risk control helps in a tough window.

### 5) Last-year exposure behavior (what the strategy is doing)
![Exposure overlay (last year)](../reports/strategy_regime_trend/plots/exposure_overlay_last_year.png)
...
![Equity curve comparison (full sample)](../reports/strategy_regime_trend/plots/equity_curve_compare.png)

What to notice:
- **Over the full history, regime-trend stays well above buy-and-hold.**
- Trend-only ends higher but takes larger drawdowns along the way.

### 7) Alpha and beta over time (last 12 months)
![Rolling alpha/beta (last year)](../reports/strategy_regime_trend/plots/rolling_alpha_beta_last_year.png)
...
- **Even at 10 bps per turnover, regime-trend still holds a meaningful edge.**

### 9) Simple forward-looking stress test (GBM)
![GBM alpha distribution (3Y)](../reports/strategy_regime_trend_gbm/plots/gbm_alpha_dist_3y.png)
...
Regime-Trend strategy results (this backtest):
...
**The regime-trend strategy (trend + regime) is the best balance in this sample:**
...
much return. **Regime-Trend is the middle ground that still wins.**
